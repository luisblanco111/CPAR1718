...
int main(int argc, char *argv[]) {
	...
	float *atenuaciones = (float *)malloc( sizeof(float) * layer_size );
	if ( layer == NULL || layer_copy == NULL || atenuaciones == NULL) {
	...
	for( i=0; i<num_storms; i++) {
		...
		for( j=0; j<storms[i].size; j++ ) {
			...
			for( k=0; k<layer_size; k++ ) {
				...
				/* 3. Raiz cuadrada de la distancia */
				float atenuacion; 
				if(atenuaciones[distancia]==0){
					atenuacion = sqrtf( (float)distancia );
					atenuaciones[distancia]=atenuacion;
				} else{
					atenuacion = atenuaciones[distancia];
				}
